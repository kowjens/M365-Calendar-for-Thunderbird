...
function updateBuildModeBanner() {
  ...
  const version = displayVersion(state.auth.version || "2.0.49");
...
function diagnosticReadme(data) {
  ...
  return [
    "M365 Calendar V2.49 - diagnostic export",
    "",
    "Purpose:",
    ...
    "- native_current_view.json: current Thunderbird Month/Multiweek/Week/Day view, exact range queries and best-effort rendered-item scan",
    "- native_provider_queries.json: bounded history of real getItems() requests received by the M365 provider",
...
async function exportCalendarDiagnostics() {
  ...
  try {
    ...
    const files = [
      ...
      { name: "native_current_view.json", data: diagnosticJson(nativeSnapshot?.currentViewDiagnostics || {}) },
      { name: "native_provider_queries.json", data: diagnosticJson(nativeSnapshot?.providerQueryStats || data?.nativeDiagnostics?.providerQueryStats || {}) },
    ...
    downloadDiagnosticBlob(zip, `M365_Calendar_Diagnostics_V2.49_${stamp}.zip`);
    const summary = data?.comparison?.summary || {};
...
async function openSettings() {
  ...
  const distribution = t("buildInfoPublic", displayVersion(state.auth.version || "2.0.49"));
...
function renderNativeDiagnostics(status) {
  ...
  const lines = [
    `build=${state.auth?.nativeMode ? "NATIVE" : "STANDARD"} ${state.auth?.version || "2.0.49"}`,
...
async function refreshNativeStatus() {
  ...
  try {
    ...
    if (els.nativeDebugText) {
      els.nativeDebugText.textContent = `build=${state.auth?.nativeMode ? "NATIVE" : "STANDARD"} ${state.auth?.version || "2.0.49"}\nFAIL refreshNativeStatus :: ${error.message || String(error)}`;
