...
async function loadSettings(){
  ...
  $("clientId").value=cfg.clientId||"";$("tenant").value=cfg.tenant||"";$("timeZone").value=cfg.timeZone||"W. Europe Standard Time";$("autoSyncMinutes").value=String(cfg.autoSyncMinutes??5);$("nativeIntegration").checked=cfg.nativeIntegration!==false;$("confirmOutgoingMessages").checked=cfg.confirmOutgoingMessages!==false;$("externalImipFallback").checked=cfg.externalImipFallback===true;
...
async function refreshDiagnostics(){
  ...
  try{const pong=await withTimeout(browser.nativeCalendar?.ping?.(),3000,"nativeCalendar.ping");state("experimentState",pong?"loaded / ping OK":"unavailable",Boolean(pong));lines.push(`experimentPing=${Boolean(pong)}`);}catch(error){state("experimentState",`ERROR: ${error.message}`,false);lines.push(`experimentError=${error.stack||error}`);}
