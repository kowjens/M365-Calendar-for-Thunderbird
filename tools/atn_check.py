...
for variant,expected in EXPECTED.items():
    ...
    if m.get('version')!='2.0.39': errors.append(f'{variant}: version must be 2.0.39')
    ...
    if g.get('strict_max_version') is not None: errors.append(f'{variant}: GitHub source manifest must not impose strict_max_version in V2.39')
...
if mod.ATN_STRICT_MAX_VERSION!='156.*': errors.append('ATN strict max must be 156.* for V2.39 submission package')
if errors:
    print('ATN/public preflight failed:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('ATN/public preflight passed (GitHub uncapped; ATN NATIVE packages at 156.*).')
